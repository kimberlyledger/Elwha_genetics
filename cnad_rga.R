...
##Load data
# Genetic data
cn_adaptive_gd_pop <- readRDS("popdist_post_adp_samplingsite_Chinook.rds")
cn_adaptive_gd_pop <- as.matrix(cn_adaptive_gd_pop)
...
# Coordinates data (dropping SBLR population of Steelhead)
...
cn_adaptive_gd_pop_ord <- cn_adaptive_gd_pop[order(factor(rownames(cn_adaptive_gd_pop), levels = cn_order)),order(factor(colnames(cn_adaptive_gd_pop), levels = cn_order))] 
...
#covariates_lowres_ibr_sh <- stack(FlowVel_lr, IP_Steelhd_lr, BFQ_lr, MWAT_lr, PRECIP_lr, SLOPE_lr, CANOPY_lr, Pool_freq_lr, logjams_10_lr, spawnable_lr) #full model covariates for Steelhead
...
#covariates_lowres_ibrsb_sh <- stack(FlowVel_lr, IP_Steelhd_lr, BFQ_lr, MWAT_lr, PRECIP_lr, SLOPE_lr, CANOPY_lr) #subset model covariates excluding "riverscape variables" which has limited range
...
#covariates_lowres_shhbt <- stack(IP_Steelhd_lr, SLOPE_lr, CANOPY_lr, Pool_freq_lr, logjams_10_lr, spawnable_lr) #steelhead habitat model covariates
#covariates_lowres_cnhbtsb <- stack(IP_Chinook_lr, SLOPE_lr, CANOPY_lr) #chinook habitat subset model covariates 
#covariates_lowres_shhbtsb <- stack(IP_Steelhd_lr, SLOPE_lr, CANOPY_lr) #steelhead habitat subset model covariates
...
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/cnad/"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/cnad/ibr/"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/cnad/ibrsb/"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/cnad/flow/"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/cnad/clim/"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/cnad/hbt/"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/cnad/hbtsb/"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/cnad/ibd/"))
cnad_ibr.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/cnad/ibr/"
cnad_ibrsb.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/cnad/ibrsb/"
cnad_flow.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/cnad/flow/"
cnad_clim.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/cnad/clim/"
cnad_hbt.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/cnad/hbt/"
cnad_hbtsb.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/cnad/hbtsb/"
cnad_ibd.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/cnad/ibd/"

# Change the number of cores below
cnad_gdist.inputs <- gdist.prep(n.Pops = length(cn.locales),
                                samples = cn.locales,
                                response = as.vector(lower(cn_adaptive_gd_pop_ord)),
                                method = 'commuteDistance')

# Change the number of cores in "parallel" below
# Chinook adaptive IBR with 1 replicate, 1 iteration (i.e., no bootstrapping)
# individual and full model
                          
cnad_ibr_GA.inputs <- GA.prep(method = "LL",
                              ASCII.dir = covariates_ibr_cn,
                              scale = FALSE,
                              Results.dir = cnad_ibr.dir,
                              parallel = 64)

#cnad_ibr_SS_results <- SS_optim(gdist.inputs = cnad_gdist.inputs,
#                                GA.inputs = cnad_ibr_GA.inputs,
#                                diagnostic_plots = FALSE)
#saveRDS(cnad_ibr_SS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_ibr_SS_results.RDS")
                                                           
cnad_ibr_MS_results <- MS_optim(gdist.inputs = cnad_gdist.inputs,
                                GA.inputs = cnad_ibr_GA.inputs,
                                diagnostic_plots = FALSE)
saveRDS(cnad_ibr_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_ibr_MS_results.RDS")
print("Full ibr model has been optimized.")

# full model (subset - excluding layers from "riverscape variables")
cnad_ibrsb_GA.inputs <- GA.prep(method = "LL",
                                ASCII.dir = covariates_ibrsb_cn,
                                scale = FALSE,
                                Results.dir = cnad_ibrsb.dir,
                                parallel = 64)
                                
cnad_ibrsb_MS_results <- MS_optim(gdist.inputs = cnad_gdist.inputs,
                                  GA.inputs = cnad_ibrsb_GA.inputs,
                                  diagnostic_plots = FALSE)
saveRDS(cnad_ibrsb_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_ibrsb_MS_results.RDS")
print("Subset ibr model has been optimized.")
                                
# flow model                            
cnad_flow_GA.inputs <- GA.prep(method = "LL",
                               ASCII.dir = covariates_flow,
                               scale = FALSE,
                               Results.dir = cnad_flow.dir,
                               parallel = 64)

cnad_flow_SS_results <- SS_optim(gdist.inputs = cnad_gdist.inputs,
                                 GA.inputs = cnad_flow_GA.inputs,
                                 diagnostic_plots = FALSE)
saveRDS(cnad_flow_SS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_flow_SS_results.RDS")
print("Flow model has been optimized.")
                                 
# clim model - failed to be optimized (all 25 interations with Best LL=-99999)                          
cnad_clim_GA.inputs <- GA.prep(method = "LL",
                               ASCII.dir = covariates_clim,
                               scale = FALSE,
                               Results.dir = cnad_clim.dir,
                               parallel = 64)

cnad_clim_MS_results <- MS_optim(gdist.inputs = cnad_gdist.inputs,
                                 GA.inputs = cnad_clim_GA.inputs,
                                 diagnostic_plots = FALSE)
saveRDS(cnad_clim_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_clim_MS_results.RDS")
print("Climate model has been optimized.")
                                 
# habitat model (full)
cnad_hbt_GA.inputs <- GA.prep(method = "LL",
                              ASCII.dir = covariates_cnhbt,
                              scale = FALSE,
                              Results.dir = cnad_hbt.dir,
                              parallel = 64)

cnad_hbt_MS_results <- MS_optim(gdist.inputs = cnad_gdist.inputs,
                                GA.inputs = cnad_hbt_GA.inputs,
                                diagnostic_plots = FALSE)
saveRDS(cnad_hbt_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_hbt_MS_results.RDS")
print("Habitat model has been optimized.")
                                 
# habitat model (subset)
cnad_hbtsb_GA.inputs <- GA.prep(method = "LL",
                                ASCII.dir = covariates_cnhbtsb,
                                scale = FALSE,
                                Results.dir = cnad_hbtsb.dir,
                                parallel = 64)

cnad_hbtsb_MS_results <- MS_optim(gdist.inputs = cnad_gdist.inputs,
                                  GA.inputs = cnad_hbtsb_GA.inputs,
                                  diagnostic_plots = FALSE)
saveRDS(cnad_hbtsb_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_hbtsb_MS_results.RDS")
print("Subset habitat model has been optimized.")
                                 
# Chinook adaptive IBD with 1 replicate, 1 iteration (i.e., no bootstrapping)
cnad_ibd_GA.inputs <- GA.prep(method = "LL",
                              ASCII.dir = covariates_ibd,
                              scale = FALSE,
                              Results.dir = cnad_ibd.dir,
                              parallel = 64)                              

cnad_ibd_results <- SS_optim(gdist.inputs = cnad_gdist.inputs,
                             GA.inputs = cnad_ibd_GA.inputs,
                             diagnostic_plots = FALSE) 
saveRDS(cnad_ibd_results, file="/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_ibd_results.RDS")
...
cnad_ibr_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_ibr_MS_results.RDS")
cnad_ibrsb_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_ibrsb_MS_results.RDS")
cnad_flow_SS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_flow_SS_results.RDS")
cnad_clim_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_clim_MS_results.RDS")
cnad_hbt_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_hbt_MS_results.RDS")
cnad_hbtsb_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_hbtsb_MS_results.RDS")
cnad_ibd_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_ibd_results.RDS")

cnad_mat.list <- c(cnad_ibr_MS_results$cd,
                   cnad_ibrsb_MS_results$cd,
                   cnad_flow_SS_results$cd,
                   cnad_clim_MS_results$cd,
                   cnad_hbt_MS_results$cd,
                   cnad_hbtsb_MS_results$cd,
                   cnad_ibd_results$cd)

cnad_k <- rbind(cnad_ibr_MS_results$k,
                cnad_ibrsb_MS_results$k,
                cnad_flow_SS_results$k,
                cnad_clim_MS_results$k,
                cnad_hbt_MS_results$k,
                cnad_hbtsb_MS_results$k,
                cnad_ibd_results$k)

# Create square distance matrix for response for use with
# the bootstrap function        
cnad_response <- matrix(0, 10, 10) #number of populations
cnad_response[lower.tri(cnad_response)] <- lower(cn_adaptive_gd_pop_ord)

# Run bootstrap
(cnad_AIC.boot <- Resist.boot(mod.names = names(cnad_mat.list),
                              dist.mat = cnad_mat.list,
                              n.parameters = cnad_k[,2],
                              sample.prop = 0.75,
                              iters = 1000,
                              obs = 10,
                              genetic.mat = cnad_response))
saveRDS(cnad_AIC.boot, file = "/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_AIC_result.RDS")
write.table(cnad_AIC.boot, file = "/scratch/bell/jeon96/LGC/IBR/rga/cnad/cnad_AIC_result.txt")
print("Bootstrapping has been finished.")
print("All RGA anayses has been finished for Chinook adaptive genetic distance response.")