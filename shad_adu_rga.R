...
shadu_adaptive_gd_pop <- readRDS("gen_steel_post_adp_adult.rds")
shadu_adaptive_gd_pop <- as.matrix(shadu_adaptive_gd_pop)
...
shadu_adaptive_gd_pop_ord <- shadu_adaptive_gd_pop[order(factor(rownames(shadu_adaptive_gd_pop), levels = shadu_order)),order(factor(colnames(shadu_adaptive_gd_pop), levels = shadu_order))] 
...
## Steelhead_adaptive_adult
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/ibr"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/ibrsb"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/flow"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/clim"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/hbt"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/hbtsb"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/ibd"))
shad_adu_ibr.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/ibr/"
shad_adu_ibrsb.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/ibrsb/"
shad_adu_flow.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/flow/"
shad_adu_clim.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/clim/"
shad_adu_hbt.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/hbt/"
shad_adu_hbtsb.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/hbtsb/"
shad_adu_ibd.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/ibd/"

# Change the number of cores below
shad_adu_gdist.inputs <- gdist.prep(n.Pops = length(shadu.locales),
                                    samples = shadu.locales,
                                    response = as.vector(lower(shadu_adaptive_gd_pop_ord)),
                                    method = 'commuteDistance')

# Change the number of cores in "parallel" below
# Steelhead adults adaptive IBR with 1 replicate, 1 iteration (i.e., no bootstrapping)
# individual and full model
shad_adu_ibr_GA.inputs <- GA.prep(method = "LL",
                                  ASCII.dir = covariates_ibr_sh,
                                  scale = FALSE,
                                  Results.dir = shad_adu_ibr.dir,
                                  parallel = 64)

#shad_adu_ibr_SS_results <- SS_optim(gdist.inputs = shad_adu_gdist.inputs,
#                                    GA.inputs = shad_adu_ibr_GA.inputs,
#                                    diagnostic_plots = FALSE)
#saveRDS(shad_adu_ibr_SS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_ibr_SS_results.RDS")
                                                           
shad_adu_ibr_MS_results <- MS_optim(gdist.inputs = shad_adu_gdist.inputs,
                                    GA.inputs = shad_adu_ibr_GA.inputs,
                                    diagnostic_plots = FALSE)
saveRDS(shad_adu_ibr_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_ibr_MS_results.RDS")
print("Full ibr model has been optimized.")

# full model (subset - excluding layers from "riverscape variables")
shad_adu_ibrsb_GA.inputs <- GA.prep(method = "LL",
                                    ASCII.dir = covariates_ibrsb_sh,
                                    scale = FALSE,
                                    Results.dir = shad_adu_ibrsb.dir,
                                    parallel = 64)
                                
shad_adu_ibrsb_MS_results <- MS_optim(gdist.inputs = shad_adu_gdist.inputs,
                                      GA.inputs = shad_adu_ibrsb_GA.inputs,
                                      diagnostic_plots = FALSE)
saveRDS(shad_adu_ibrsb_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_ibrsb_MS_results.RDS")
print("Subset ibr model has been optimized.")
                                
# flow model                            
shad_adu_flow_GA.inputs <- GA.prep(method = "LL",
                                   ASCII.dir = covariates_flow,
                                   scale = FALSE,
                                   Results.dir = shad_adu_flow.dir,
                                   parallel = 64)

shad_adu_flow_SS_results <- SS_optim(gdist.inputs = shad_adu_gdist.inputs,
                                     GA.inputs = shad_adu_flow_GA.inputs,
                                     diagnostic_plots = FALSE)
saveRDS(shad_adu_flow_SS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_flow_SS_results.RDS")
print("Flow model has been optimized.")
                                 
# clim model - failed to be optimized (all 25 interations with Best LL=-99999)                          
shad_adu_clim_GA.inputs <- GA.prep(method = "LL",
                                   ASCII.dir = covariates_clim,
                                   scale = FALSE,
                                   Results.dir = shad_adu_clim.dir,
                                   parallel = 64)

shad_adu_clim_MS_results <- MS_optim(gdist.inputs = shad_adu_gdist.inputs,
                                     GA.inputs = shad_adu_clim_GA.inputs,
                                     diagnostic_plots = FALSE)
saveRDS(shad_adu_clim_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_clim_MS_results.RDS")
print("Climate model has been optimized.")
                                 
# habitat model (full)
shad_adu_hbt_GA.inputs <- GA.prep(method = "LL",
                                  ASCII.dir = covariates_shhbt,
                                  scale = FALSE,
                                  Results.dir = shad_adu_hbt.dir,
                                  parallel = 64)

shad_adu_hbt_MS_results <- MS_optim(gdist.inputs = shad_adu_gdist.inputs,
                                    GA.inputs = shad_adu_hbt_GA.inputs,
                                    diagnostic_plots = FALSE)
saveRDS(shad_adu_hbt_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_hbt_MS_results.RDS")
print("Habitat model has been optimized.")
                                 
# habitat model (subset)
shad_adu_hbtsb_GA.inputs <- GA.prep(method = "LL",
                                    ASCII.dir = covariates_shhbtsb,
                                    scale = FALSE,
                                    Results.dir = shad_adu_hbtsb.dir,
                                    parallel = 64)

shad_adu_hbtsb_MS_results <- MS_optim(gdist.inputs = shad_adu_gdist.inputs,
                                      GA.inputs = shad_adu_hbtsb_GA.inputs,
                                      diagnostic_plots = FALSE)
saveRDS(shad_adu_hbtsb_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_hbtsb_MS_results.RDS")
print("Subset habitat model has been optimized.")
                                 
# Steelhead adults adaptive IBD with 1 replicate, 1 iteration (i.e., no bootstrapping)
shad_adu_ibd_GA.inputs <- GA.prep(method = "LL",
                                  ASCII.dir = covariates_ibd,
                                  scale = FALSE,
                                  Results.dir = shad_adu_ibd.dir,
                                  parallel = 64)                              

shad_adu_ibd_results <- SS_optim(gdist.inputs = shad_adu_gdist.inputs,
                                 GA.inputs = shad_adu_ibd_GA.inputs,
                                 diagnostic_plots = FALSE) 
saveRDS(shad_adu_ibd_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_ibd_results.RDS")
...
shad_adu_ibr_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_ibr_MS_results.RDS")
shad_adu_ibrsb_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_ibrsb_MS_results.RDS")
shad_adu_flow_SS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_flow_SS_results.RDS")
shad_adu_clim_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_clim_MS_results.RDS")
shad_adu_hbt_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_hbt_MS_results.RDS")
shad_adu_hbtsb_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_hbtsb_MS_results.RDS")
shad_adu_ibd_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_ibd_results.RDS")

shad_adu_mat.list <- c(shad_adu_ibr_MS_results$cd,
                       shad_adu_ibrsb_MS_results$cd,
                       shad_adu_flow_SS_results$cd,
                       shad_adu_clim_MS_results$cd,
                       shad_adu_hbt_MS_results$cd,
                       shad_adu_hbtsb_MS_results$cd,
                       shad_adu_ibd_results$cd)

shad_adu_k <- rbind(shad_adu_ibr_MS_results$k,
                    shad_adu_ibrsb_MS_results$k,
                    shad_adu_flow_SS_results$k,
                    shad_adu_clim_MS_results$k,
                    shad_adu_hbt_MS_results$k,
                    shad_adu_hbtsb_MS_results$k,
                    shad_adu_ibd_results$k)

# Create square distance matrix for response for use with
# the bootstrap function        
shad_adu_response <- matrix(0, 16, 16) #number of populations
shad_adu_response[lower.tri(shad_adu_response)] <- lower(shadu_adaptive_gd_pop_ord)

# Run bootstrap
(shad_adu_AIC.boot <- Resist.boot(mod.names = names(shad_adu_mat.list),
                                  dist.mat = shad_adu_mat.list,
                                  n.parameters = shad_adu_k[,2],
                                  sample.prop = 0.75,
                                  iters = 1000,
                                  obs = 16,
                                  genetic.mat = shad_adu_response))
saveRDS(shad_adu_AIC.boot, file = "/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_AIC_result.RDS")
write.table(shad_adu_AIC.boot, file = "/scratch/bell/jeon96/LGC/IBR/rga/shad_adu/shad_adu_AIC_result.txt")
print("Bootstrapping has been finished.")
print("All RGA anayses has been finished for Steelhead adults adaptive genetic distance response.")