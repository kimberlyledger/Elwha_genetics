...
shjuv_adaptive_gd_pop <- readRDS("gen_steel_post_adp_juvenile.rds")
shjuv_adaptive_gd_pop <- as.matrix(shjuv_adaptive_gd_pop)
...
shjuv_adaptive_gd_pop_ord <- shjuv_adaptive_gd_pop[order(factor(rownames(shjuv_adaptive_gd_pop), levels = shjuv_order)),order(factor(colnames(shjuv_adaptive_gd_pop), levels = shjuv_order))] 
...
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/ibr"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/ibrsb"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/flow"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/clim"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/hbt"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/hbtsb"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/ibd"))
shad_juv_ibr.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/ibr/"
shad_juv_ibrsb.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/ibrsb/"
shad_juv_flow.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/flow/"
shad_juv_clim.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/clim/"
shad_juv_hbt.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/hbt/"
shad_juv_hbtsb.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/hbtsb/"
shad_juv_ibd.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/ibd/"

# Change the number of cores below
shad_juv_gdist.inputs <- gdist.prep(n.Pops = length(shjuv.locales),
                                    samples = shjuv.locales,
                                    response = as.vector(lower(shjuv_adaptive_gd_pop_ord)),
                                    method = 'commuteDistance')

# Change the number of cores in "parallel" below
# Steelhead juveniles adaptive IBR with 1 replicate, 1 iteration (i.e., no bootstrapping)
# individual and full model
shad_juv_ibr_GA.inputs <- GA.prep(method = "LL",
                                  ASCII.dir = covariates_ibr_sh,
                                  scale = FALSE,
                                  Results.dir = shad_juv_ibr.dir,
                                  parallel = 64)
...
shad_juv_ibr_MS_results <- MS_optim(gdist.inputs = shad_juv_gdist.inputs,
                                    GA.inputs = shad_juv_ibr_GA.inputs,
                                    diagnostic_plots = FALSE)
saveRDS(shad_juv_ibr_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_ibr_MS_results.RDS")
print("Full ibr model has been optimized.")

# full model (subset - excluding layers from "riverscape variables")
shad_juv_ibrsb_GA.inputs <- GA.prep(method = "LL",
                                    ASCII.dir = covariates_ibrsb_sh,
                                    scale = FALSE,
                                    Results.dir = shad_juv_ibrsb.dir,
                                    parallel = 64)
                                
shad_juv_ibrsb_MS_results <- MS_optim(gdist.inputs = shad_juv_gdist.inputs,
                                      GA.inputs = shad_juv_ibrsb_GA.inputs,
                                      diagnostic_plots = FALSE)
saveRDS(shad_juv_ibrsb_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_ibrsb_MS_results.RDS")
print("Subset ibr model has been optimized.")
                                
# flow model                            
shad_juv_flow_GA.inputs <- GA.prep(method = "LL",
                                   ASCII.dir = covariates_flow,
                                   scale = FALSE,
                                   Results.dir = shad_juv_flow.dir,
                                   parallel = 64)

shad_juv_flow_SS_results <- SS_optim(gdist.inputs = shad_juv_gdist.inputs,
                                     GA.inputs = shad_juv_flow_GA.inputs,
                                     diagnostic_plots = FALSE)
saveRDS(shad_juv_flow_SS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_flow_SS_results.RDS")
print("Flow model has been optimized.")
                                 
# clim model - failed to be optimized (all 25 interations with Best LL=-99999)                          
shad_juv_clim_GA.inputs <- GA.prep(method = "LL",
                                   ASCII.dir = covariates_clim,
                                   scale = FALSE,
                                   Results.dir = shad_juv_clim.dir,
                                   parallel = 64)

shad_juv_clim_MS_results <- MS_optim(gdist.inputs = shad_juv_gdist.inputs,
                                     GA.inputs = shad_juv_clim_GA.inputs,
                                     diagnostic_plots = FALSE)
saveRDS(shad_juv_clim_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_clim_MS_results.RDS")
print("Climate model has been optimized.")
                                 
# habitat model (full)
shad_juv_hbt_GA.inputs <- GA.prep(method = "LL",
                                  ASCII.dir = covariates_shhbt,
                                  scale = FALSE,
                                  Results.dir = shad_juv_hbt.dir,
                                  parallel = 64)

shad_juv_hbt_MS_results <- MS_optim(gdist.inputs = shad_juv_gdist.inputs,
                                    GA.inputs = shad_juv_hbt_GA.inputs,
                                    diagnostic_plots = FALSE)
saveRDS(shad_juv_hbt_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_hbt_MS_results.RDS")
print("Habitat model has been optimized.")
                                 
# habitat model (subset)
shad_juv_hbtsb_GA.inputs <- GA.prep(method = "LL",
                                    ASCII.dir = covariates_shhbtsb,
                                    scale = FALSE,
                                    Results.dir = shad_juv_hbtsb.dir,
                                    parallel = 64)

shad_juv_hbtsb_MS_results <- MS_optim(gdist.inputs = shad_juv_gdist.inputs,
                                      GA.inputs = shad_juv_hbtsb_GA.inputs,
                                      diagnostic_plots = FALSE)
saveRDS(shad_juv_hbtsb_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_hbtsb_MS_results.RDS")
print("Subset habitat model has been optimized.")
                                 
# Steelhead juveniles adaptive IBD with 1 replicate, 1 iteration (i.e., no bootstrapping)
shad_juv_ibd_GA.inputs <- GA.prep(method = "LL",
                                  ASCII.dir = covariates_ibd,
                                  scale = FALSE,
                                  Results.dir = shad_juv_ibd.dir,
                                  parallel = 64)                              

shad_juv_ibd_results <- SS_optim(gdist.inputs = shad_juv_gdist.inputs,
                                 GA.inputs = shad_juv_ibd_GA.inputs,
                                 diagnostic_plots = FALSE) 
saveRDS(shad_juv_ibd_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_ibd_results.RDS")
...
shad_juv_ibr_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_ibr_MS_results.RDS")
shad_juv_ibrsb_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_ibrsb_MS_results.RDS")
shad_juv_flow_SS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_flow_SS_results.RDS")
shad_juv_clim_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_clim_MS_results.RDS")
shad_juv_hbt_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_hbt_MS_results.RDS")
shad_juv_hbtsb_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_hbtsb_MS_results.RDS")
shad_juv_ibd_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_ibd_results.RDS")

shad_juv_mat.list <- c(shad_juv_ibr_MS_results$cd,
                       shad_juv_ibrsb_MS_results$cd,
                       shad_juv_flow_SS_results$cd,
                       shad_juv_clim_MS_results$cd,
                       shad_juv_hbt_MS_results$cd,
                       shad_juv_hbtsb_MS_results$cd,
                       shad_juv_ibd_results$cd)

shad_juv_k <- rbind(shad_juv_ibr_MS_results$k,
                    shad_juv_ibrsb_MS_results$k,
                    shad_juv_flow_SS_results$k,
                    shad_juv_clim_MS_results$k,
                    shad_juv_hbt_MS_results$k,
                    shad_juv_hbtsb_MS_results$k,
                    shad_juv_ibd_results$k)

# Create square distance matrix for response for use with
# the bootstrap function        
shad_juv_response <- matrix(0, 4, 4) #number of populations
shad_juv_response[lower.tri(shad_juv_response)] <- lower(shjuv_adaptive_gd_pop_ord)

# Run bootstrap
(shad_juv_AIC.boot <- Resist.boot(mod.names = names(shad_juv_mat.list),
                                  dist.mat = shad_juv_mat.list,
                                  n.parameters = shad_juv_k[,2],
                                  sample.prop = 0.75,
                                  iters = 1000,
                                  obs = 4,
                                  genetic.mat = shad_juv_response))
saveRDS(shad_juv_AIC.boot, file = "/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_AIC_result.RDS")
write.table(shad_juv_AIC.boot, file = "/scratch/bell/jeon96/LGC/IBR/rga/shad_juv/shad_juv_AIC_result.txt")
print("Bootstrapping has been finished.")
print("All RGA anayses has been finished for Steelhead/rainbow trout juvenile adaptive genetic distance response.")