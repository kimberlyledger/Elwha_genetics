...
sh_neutral_gd_pop <- readRDS("gen_steel_post_neutral.rds")
sh_neutral_gd_pop <- as.matrix(sh_neutral_gd_pop)
...
sh_neutral_gd_pop_ord <- sh_neutral_gd_pop[order(factor(rownames(sh_neutral_gd_pop), levels = sh_order)),order(factor(colnames(sh_neutral_gd_pop), levels = sh_order))] 
...
## Steelhead_all_neutral
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shnt/"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shnt/ibr"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shnt/ibrsb"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shnt/flow"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shnt/clim"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shnt/hbt"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shnt/hbtsb"))
dir.create(file.path("/scratch/bell/jeon96/LGC/IBR/rga/shnt/ibd"))
shnt_ibr.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shnt/ibr/"
shnt_ibrsb.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shnt/ibrsb/"
shnt_flow.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shnt/flow/"
shnt_clim.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shnt/clim/"
shnt_hbt.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shnt/hbt/"
shnt_hbtsb.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shnt/hbtsb/"
shnt_ibd.dir <- "/scratch/bell/jeon96/LGC/IBR/rga/shnt/ibd/"

# Change the number of cores below
shnt_gdist.inputs <- gdist.prep(n.Pops = length(sh.locales),
                                samples = sh.locales,
                                response = as.vector(lower(sh_neutral_gd_pop_ord)),
                                method = 'commuteDistance')

# Change the number of cores in "parallel" below
# Steelhead neutral IBR with 1 replicate, 1 iteration (i.e., no bootstrapping)
# individual and full model
#print("Full IBR model optimization started.")
shnt_ibr_GA.inputs <- GA.prep(method = "LL",
                              ASCII.dir = covariates_ibr_sh,
                              scale = FALSE,
                              Results.dir = shnt_ibr.dir,
                              parallel = 64)

#shnt_ibr_SS_results <- SS_optim(gdist.inputs = shnt_gdist.inputs,
#                                GA.inputs = shnt_ibr_GA.inputs,
#                                diagnostic_plots = FALSE)
#saveRDS(shnt_ibr_SS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_ibr_SS_results.RDS")
                                                           
shnt_ibr_MS_results <- MS_optim(gdist.inputs = shnt_gdist.inputs,
                                GA.inputs = shnt_ibr_GA.inputs,
                                diagnostic_plots = FALSE)
saveRDS(shnt_ibr_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_ibr_MS_results.RDS")
...
shnt_ibrsb_GA.inputs <- GA.prep(method = "LL",
                                ASCII.dir = covariates_ibrsb_sh,
                                scale = FALSE,
                                Results.dir = shnt_ibrsb.dir,
                                parallel = 64)
                                
shnt_ibrsb_MS_results <- MS_optim(gdist.inputs = shnt_gdist.inputs,
                                  GA.inputs = shnt_ibrsb_GA.inputs,
                                  diagnostic_plots = FALSE)
saveRDS(shnt_ibrsb_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_ibrsb_MS_results.RDS")
print("Subset IBR model has been optimized.")
                                
# flow model   
#print("Flow model optimization started.")                          
shnt_flow_GA.inputs <- GA.prep(method = "LL",
                               ASCII.dir = covariates_flow,
                               scale = FALSE,
                               Results.dir = shnt_flow.dir,
                               parallel = 64)

shnt_flow_SS_results <- SS_optim(gdist.inputs = shnt_gdist.inputs,
                                 GA.inputs = shnt_flow_GA.inputs,
                                 diagnostic_plots = FALSE)
saveRDS(shnt_flow_SS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_flow_SS_results.RDS")
print("Flow model has been optimized.")
                                 
# clim model
#print("Climate model optimization started.")                         
shnt_clim_GA.inputs <- GA.prep(method = "LL",
                               ASCII.dir = covariates_clim,
                               scale = FALSE,
                               Results.dir = shnt_clim.dir,
                               parallel = 64)

shnt_clim_MS_results <- MS_optim(gdist.inputs = shnt_gdist.inputs,
                                 GA.inputs = shnt_clim_GA.inputs,
                                 diagnostic_plots = FALSE)
saveRDS(shnt_clim_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_clim_MS_results.RDS")
print("Climate model has been optimized.")
                                 
# habitat model (full)
#print("Habitat model optimization started.") 
shnt_hbt_GA.inputs <- GA.prep(method = "LL",
                              ASCII.dir = covariates_shhbt,
                              scale = FALSE,
                              Results.dir = shnt_hbt.dir,
                              parallel = 64)

shnt_hbt_MS_results <- MS_optim(gdist.inputs = shnt_gdist.inputs,
                                GA.inputs = shnt_hbt_GA.inputs,
                                diagnostic_plots = FALSE)
saveRDS(shnt_hbt_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_hbt_MS_results.RDS")
print("Habitat model has been optimized.")
                                 
# habitat model (subset)
#print("Subset habitat model optimization started.") 
shnt_hbtsb_GA.inputs <- GA.prep(method = "LL",
                                ASCII.dir = covariates_shhbtsb,
                                scale = FALSE,
                                Results.dir = shnt_hbtsb.dir,
                                parallel = 64)

shnt_hbtsb_MS_results <- MS_optim(gdist.inputs = shnt_gdist.inputs,
                                  GA.inputs = shnt_hbtsb_GA.inputs,
                                  diagnostic_plots = FALSE)
saveRDS(shnt_hbtsb_MS_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_hbtsb_MS_results.RDS")
print("Subset habitat model has been optimized.")
                                 
# Steelhead neutral IBD with 1 replicate, 1 iteration (i.e., no bootstrapping)
#print("IBD model optimization started.") 
shnt_ibd_GA.inputs <- GA.prep(method = "LL",
                              ASCII.dir = covariates_ibd,
                              scale = FALSE,
                              Results.dir = shnt_ibd.dir,
                              parallel = 64)                              

shnt_ibd_results <- SS_optim(gdist.inputs = shnt_gdist.inputs,
                             GA.inputs = shnt_ibd_GA.inputs,
                             diagnostic_plots = FALSE) 
saveRDS(shnt_ibd_results, file="/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_ibd_results.RDS")
...
shnt_ibr_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_ibr_MS_results.RDS")
shnt_ibrsb_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_ibrsb_MS_results.RDS")
shnt_flow_SS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_flow_SS_results.RDS")
shnt_clim_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_clim_MS_results.RDS")
shnt_hbt_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_hbt_MS_results.RDS")
shnt_hbtsb_MS_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_hbtsb_MS_results.RDS")
shnt_ibd_results <- readRDS("/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_ibd_results.RDS")

shnt_mat.list <- c(#shnt_ibr_MS_results$cd,
                   shnt_ibrsb_MS_results$cd,
                   shnt_flow_SS_results$cd,
                   shnt_clim_MS_results$cd,
                   shnt_hbt_MS_results$cd,
                   shnt_hbtsb_MS_results$cd,
                   shnt_ibd_results$cd)

shnt_k <- rbind(#shnt_ibr_MS_results$k,
                shnt_ibrsb_MS_results$k,
                shnt_flow_SS_results$k,
                shnt_clim_MS_results$k,
                shnt_hbt_MS_results$k,
                shnt_hbtsb_MS_results$k,
                shnt_ibd_results$k)

# Create square distance matrix for response for use with
# the bootstrap function        
shnt_response <- matrix(0, 19, 19) #number of populations
shnt_response[lower.tri(shnt_response)] <- lower(sh_neutral_gd_pop_ord)

# Run bootstrap
(shnt_AIC.boot <- Resist.boot(mod.names = names(shnt_mat.list),
                              dist.mat = shnt_mat.list,
                              n.parameters = shnt_k[,2],
                              sample.prop = 0.75,
                              iters = 1000,
                              obs = 19,
                              genetic.mat = shnt_response))
saveRDS(shnt_AIC.boot, file = "/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_AIC_result.RDS")
write.table(shnt_AIC.boot, file = "/scratch/bell/jeon96/LGC/IBR/rga/shnt/shnt_AIC_result.txt")
print("Bootstrapping has been finished.")
print("All RGA anayses has been finished for Steelhead/rainbow trout all neutral genetic distance response.")