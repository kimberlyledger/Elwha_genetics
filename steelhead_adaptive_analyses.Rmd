---
title: "steelhead_adaptive_analyses_deprecated"
...
#Loadings of each locus to PC axis.
loadingplot(steelhead_adaptive.pca, axis=1) #for PC1
loadingplot(steelhead_adaptive.pca, axis=2) #for PC2

##dudi.pca version
test<-steelhead_adaptive.genind
testing<-test$tab
f1 <- function(vec) {
...
# Encode the row names with the SNP IDs
for (i in 1:nrow(c1)){
  c1$SNP[i] <- as.character(steelhead_adaptive[(which(steelhead_adaptive$SnpPos == substr(rownames(c1)[i], 1, nchar(rownames(c1)[i])-2))),3]) #3rd column of "steelhead_adaptive" includes SnpName
}

# Now you have a file (pca.c1) with all of the loadings
write.csv(c1,"adaptivePCA_Loadings.csv")
...
#Change row order as following: Pre_AD, Post_AD, Pre_ID, Post_ID, Pre_BD, During_BD, Post_BD, Pre_SBLR, Post_SBLR
#steelhead_af_ordered <- steelhead_afSNP[c(3, 8, 1, 6, 4, 5, 6, 2, 9),]
steelhead_af_ordered <- steelhead_afSNP[c("Pre_AD", "Post_AD", "Pre_ID", "Post_ID", "Pre_BD", "During_BD", "Post_BD", "Pre_SBLR", "Post_SBLR"),]
write.table(steelhead_af_ordered, "steelhead_af.txt", sep = "\t")

steelhead_af_pruned <- read.delim("C:/Users/jyj55/Desktop/LGC-DGS_2022/Team_project/Analysis/Steelhead analysis/Elwha_genetics/steelhead_af_pruned.txt", row.names=1) #Reread manually pruned table to have major allele frequency only for each locus (pruning to remain only one allele per locus)
...
steelhead_runtimeSNP.gt.vcf <- steelhead_adaptiveSNP.gt.vcf %>% select(any_of(Sample_ID, Omy28_11607954
, "Omy_RAD52458-17", Omy_GREB1_05, Omy28_11625241, Omy28_11632591, Omy_GREB1_09, Omy28_11658853, Omy28_11667578, "Omy_RAD47080-54", Omy28_11671116, Omy28_11676622, Omy28_11683204, Omy28_11773194)) #Remain loci associated with run-timing

steelhead_runtimeSNP.gt.vcf_PreAD <- steelhead_runtimeSNP.gt.vcf[steelhead_runtimeSNP.gt.vcf$Time_Location == "Pre_AD",]
steelhead_runtimeSNP.gt.vcf_PostAD <- steelhead_runtimeSNP.gt.vcf[steelhead_runtimeSNP.gt.vcf$Time_Location == "Post_AD",]
steelhead_runtimeSNP.gt.vcf_PreID <- steelhead_runtimeSNP.gt.vcf[steelhead_runtimeSNP.gt.vcf$Time_Location == "Pre_ID",]
steelhead_runtimeSNP.gt.vcf_PostID <- steelhead_runtimeSNP.gt.vcf[steelhead_runtimeSNP.gt.vcf$Time_Location == "Post_ID",]
steelhead_runtimeSNP.gt.vcf_PreBD <- steelhead_runtimeSNP.gt.vcf[steelhead_runtimeSNP.gt.vcf$Time_Location == "Pre_BD",]
steelhead_runtimeSNP.gt.vcf_DuringBD <- steelhead_runtimeSNP.gt.vcf[steelhead_runtimeSNP.gt.vcf$Time_Location == "During_BD",]
steelhead_runtimeSNP.gt.vcf_PostBD <- steelhead_runtimeSNP.gt.vcf[steelhead_runtimeSNP.gt.vcf$Time_Location == "Post_BD",]
steelhead_runtimeSNP.gt.vcf_PreSBLR <- steelhead_runtimeSNP.gt.vcf[steelhead_runtimeSNP.gt.vcf$Time_Location == "Pre_SBLR",]
steelhead_runtimeSNP.gt.vcf_PostSBLR <- steelhead_runtimeSNP.gt.vcf[steelhead_runtimeSNP.gt.vcf$Time_Location == "Post_SBLR",]

steelhead_gf_PreAD <- GenotypeFreq(steelhead_runtimeSNP.gt.vcf_PreAD[,2:(ncol(steelhead_runtimeSNP.gt.vcf_PreAD)-1)],"/",expect=FALSE) #expect=FALSE to calculate "observed" genotype frequencies
...
steelhead_gf_PostAD <- GenotypeFreq(steelhead_runtimeSNP.gt.vcf_PostAD[,2:(ncol(steelhead_runtimeSNP.gt.vcf_PostAD)-1)],"/",expect=FALSE)u
...
steelhead_gf_PreID <- GenotypeFreq(steelhead_runtimeSNP.gt.vcf_PreID[,2:(ncol(steelhead_runtimeSNP.gt.vcf_PreID)-1)],"/",expect=FALSE)
steelhead_gf_PreID[] <- sapply(steelhead_gf_PreID, function(x){x/sum(x)})
steelhead_gf_PreID$Time_Location <- "Pre_ID"
steelhead_gf_PostID <- GenotypeFreq(steelhead_runtimeSNP.gt.vcf_PostID[,2:(ncol(steelhead_runtimeSNP.gt.vcf_PostID)-1)],"/",expect=FALSE)
...
steelhead_gf_PreBD <- GenotypeFreq(steelhead_runtimeSNP.gt.vcf_PreBD[,2:(ncol(steelhead_runtimeSNP.gt.vcf_PreBD)-1)],"/",expect=FALSE)
steelhead_gf_PreBD[] <- sapply(steelhead_gf_PreBD, function(x){x/sum(x)})
steelhead_gf_PreBD$Time_Location <- "Pre_BD"
steelhead_gf_DuringBD <- GenotypeFreq(steelhead_runtimeSNP.gt.vcf_DuringBD[,2:(ncol(steelhead_runtimeSNP.gt.vcf_DuringBD)-1)],"/",expect=FALSE)
steelhead_gf_DuringBD[] <- sapply(steelhead_gf_DuringBD, function(x){x/sum(x)})
steelhead_gf_DuringBD$Time_Location <- "During_BD"
steelhead_gf_PostBD <- GenotypeFreq(steelhead_runtimeSNP.gt.vcf_PostBD[,2:(ncol(steelhead_runtimeSNP.gt.vcf_PostBD)-1)],"/",expect=FALSE)
...
steelhead_gf_PreSBLR <- GenotypeFreq(steelhead_runtimeSNP.gt.vcf_PreSBLR[,2:(ncol(steelhead_runtimeSNP.gt.vcf_PreSBLR)-1)],"/",expect=FALSE)
steelhead_gf_PreSBLR[] <- sapply(steelhead_gf_PreSBLR, function(x){x/sum(x)})
steelhead_gf_PreSBLR$Time_Location <- "Pre_SBLR"
steelhead_gf_PostSBLR <- GenotypeFreq(steelhead_runtimeSNP.gt.vcf_PostSBLR[,2:(ncol(steelhead_runtimeSNP.gt.vcf_PostSBLR)-1)],"/",expect=FALSE)
steelhead_gf_PostSBLR[] <- sapply(steelhead_gf_PostSBLR, function(x){x/sum(x)})
steelhead_gf_PostSBLR$Time_Location <- "Post_SBLR"
write.table(steelhead_gf_PreSBLR, file = "steelhead_gf_PreSBLR.txt", sep = "\t")
write.table(steelhead_gf_PostSBLR, file = "steelhead_gf_PostSBLR.txt", sep = "\t")

#After this, in Excel, among all adaptive loci, manually check & remain markers based on positions on Greb1 and decide Premature/Mature genotypes according to "Chinook-and-Mykiss-Markers.xlsx". Then combine files of same Location (e.g. steelhead_gf_BD; combined PreBD + DuringBD + PostBD), because "GenotypeFreq" function removes Sample ID, we cannot know which genotype frequency is from which “Time”. But you can just make combined files in advance here too, as they retain Pop info.
```

##Plot genotype frequency heatmaps of GREB1L loci (*check summer-run/winter-run alleles and modify)
```{r}
