...
steelhead_adaptiveSNP.gt.vcf_PreSBLR <- steelhead_adaptiveSNP.gt.vcf[steelhead_adaptiveSNP.gt.vcf$Time_Location == "Pre_SBLR",]
...
steelhead_gf_PreSBLR <- GenotypeFreq(steelhead_adaptiveSNP.gt.vcf_PreSBLR[,2:(ncol(steelhead_adaptiveSNP.gt.vcf_PreSBLR)-1)],"/",expect=FALSE)
steelhead_gf_PreSBLR[] <- sapply(steelhead_gf_PostSBLR, function(x){x/sum(x)})
...
if(all(unlist(unique(x@all.names)) %in% c("A", "T", "C", "G", "-"))){
  ...
  colnames(data) <- gsub(colnames(data),pattern="\\.-",replacement=".00")
  #####
} else {
  m_type <- "msat"
}
